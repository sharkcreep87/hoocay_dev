        ...
        <div class="table-responsive">
            <table class="table bordered striped">
                ...
                <?php $__currentLoopData = $bookings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $booking): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?>
                    <tr class="<?php echo e($booking->status == 1 ? 'disabled-style' : ''); ?>">
                        <td>
                        ...
                        <td class="booking-status"><?php if($booking->completed == 1): ?> Confirmed

                        <?php elseif($booking->completed == 2): ?> Canceled
                        <?php elseif($booking->completed == 0): ?> Pending
                        <?php endif; ?></td>
                        <td>
                            <div class="icon-options">
                                <a href="#" class="confirm-button <?php echo e($booking->status == 1 ? 'disabled-style' : ''); ?>" data-id="<?php echo e($booking->id); ?>" title="<?php echo e(get_string('approve_booking')); ?>"><i class="small material-icons color-primary">done</i></a>
                                <a href="#" class="reject-button <?php echo e($booking->status == 1 ? 'disabled-style' : ''); ?>" data-id="<?php echo e($booking->id); ?>" title="<?php echo e(get_string('reject_booking')); ?>"><i class="small material-icons color-red">close</i></a>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.user-info').click(function(e){
            ...
            var token = $('.token').val();
            $.ajax({
                url: '<?php echo e(url('/owner/booking/user_details')); ?>/' + id,
                type: 'post',
                data: {_token: token},
            ...
            if(!selector.hasClass('disabled-style')) {
                bootbox.confirm({
                    ...
                    callback: function (result) {
                        if (result) {
                            $.ajax({
                                url: '<?php echo e(url('/owner/booking/activate')); ?>/' + id,
                                type: 'post',
                                data: {_token: token},
                                beforeSend: function(){
        ...
        $('.reject-button').click(function(event){
            ...
            var status = $('.booking-status', selector);
            var token = $('.token').val();
            var msg = '<div class="alert alert-danger" role="alert">Reminder! This booking has been confirm before, You will be charge for 10% penalty and will be deduct from your pending balance.</div>';
            if(!selector.hasClass('disabled-style')) {
                bootbox.confirm({
                  
                    title: '<?php echo e(get_string('confirm_action')); ?>',
                    message: '<div class="alert alert-danger" role="alert">Reminder! You will be charge for 10% penalty and will be deduct from your pending balance.</div> <br/><?php echo e(get_string('reject_booking_confirm')); ?> <br/> <div id="reason" class="form-group"><textarea name="reason" class="form-control"></textarea></div>',
                    onEscape: true,
                    backdrop: true,
                    buttons: {
                        cancel: {
                    ...
                    callback: function (result) {
                        ...
                        if (result) {
                            $.ajax({
                                url: '<?php echo e(url('/owner/booking/reject')); ?>/' + id,
                                type: 'post',
