...
<div class="col s12">
    @if(!$errors->isEmpty())
        <span class="wrong-error">{{$errors->first()}}</span>
    @endif
     
    {!!Form::open(['method' => 'post', 'url' => route('owner.property.store'), 'files' => 'true'])!!}
        <div class="panel">
                            ...
                            <div class="tab-content">
                             <div class="col s12">
                                            <div class="form-group">
                                                {{Form::text('ref_no', null, ['class' => 'form-control', 'placeholder' => 'Introducer Code (Optional)'])}}
                                                {{Form::label('ref_no', 'Introducer Code (Optional)')}}
                                    ...
                                    <div id="lang{{$language->id}}" class="tab-pane {{$language->default ? 'active' : ''}}">
                                        <div class="col s12">
                                            <div class="form-group  {{$errors->has('name.'.$language->id.'') ? 'has-error' : ''}}">
                                                {{Form::text('name', null, ['class' => 'form-control', 'placeholder' => get_string('enter_property_name')])}}
                                        ...
                                        <span class="wrong-error">* Minimum 100 words</span>
                                            {{Form::textarea('description', null, ['class' => 'hidden desc-content form-control','id'=> 'desc'])}}
                    ...
                    <div class="col s12 clearfix">
                        <h5 class="section-title">{{get_string('media')}}</h5>
                          @if($errors->has('images'))
                                <span class="wrong-error">* {{$errors->first('images')}}</span>
                            @endif
                    </div>
        ...
        $(document).ready(function(){
            ...
            $('#file-dropzone').dropzone({
                ...
                alert(Session::get('image') );
                        ...
                        @foreach(Session::get('image') as $images)
                            var mockFile = { name: '{{ $images }}', size: 100000 };
                            this.emit("addedfile", mockFile);
                            this.createThumbnailFromUrl(mockFile, '/images/data/{{ $images }}');
                            this.emit("success", mockFile);
                        $('.hidden-fields').append('<input type="hidden" name="images[]" value="{{ $images}}">');
                        @endforeach
                    @endif*/
