        ...
        <div class="table-responsive">
            <table class="table bordered striped">
                ...
                @foreach($bookings as $booking)
                    <tr class="{{ $booking->status == 1 ? 'disabled-style' : '' }}">
                        <td>
                        ...
                        <td class="booking-status">@if ($booking->completed == 1) Confirmed

                        @elseif($booking->completed == 2) Canceled
                        @elseif($booking->completed == 0) Pending
                        @endif</td>
                        <td>
                            <div class="icon-options">
                                <a href="#" class="confirm-button {{ $booking->status == 1 ? 'disabled-style' : '' }}" data-id="{{$booking->id}}" title="{{get_string('approve_booking')}}"><i class="small material-icons color-primary">done</i></a>
                                <a href="#" class="reject-button {{ $booking->status == 1 ? 'disabled-style' : '' }}" data-id="{{$booking->id}}" title="{{get_string('reject_booking')}}"><i class="small material-icons color-red">close</i></a>
        ...
        $('.reject-button').click(function(event){
            ...
            if(!selector.hasClass('disabled-style')) {
                bootbox.confirm({
                    ...
                    title: '{{get_string('confirm_action')}}',
                    message: '<div class="alert alert-danger" role="alert">Reminder! You will be charge for 10% penalty and will be deduct from your pending balance.</div> <br/>{{get_string('reject_booking_confirm')}} <br/> <div id="reason" class="form-group"><textarea name="reason" class="form-control"></textarea></div>',
                    onEscape: true,
                    backdrop: true,
                    buttons: {
                        cancel: {
