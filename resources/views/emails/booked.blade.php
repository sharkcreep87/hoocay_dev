                        ...
                        <table style="{{ $style['email-body_inner'] }}" align="center" width="570" cellpadding="0" cellspacing="0">
                            <tr>
                                <td style="{{ $style['email-body_cell'] }}">
                                    ...
                                    @if($mail_data['isOwner'] == 1)
                                    <p style="{{ $style['paragraph'] }}">
                                        <strong>Please contact this guest as soon as posible for futher action.</strong><br/>
                                      
                                    </p>
                                    @endif
                                  
                                    <p style="{{ $style['paragraph'] }}">
                                          <strong>Booking Number : {{$mail_data['ref']}}</strong><br/>
                                        ...
                                        {{ $mail_data['str_guest_number'] }}: {{ $mail_data['guest_number'] }}<br>
                                        Booking Name : {{$mail_data['user_data']['first_name']}}<br>
                                        Phone Number : {{$mail_data['user_data']['phone']}}<br><br>
                                     
                                    </p>
                                        @if($mail_data['isOwner'] == 0)
                                    <p style="{{ $style['paragraph'] }}">
                                        <strong>If you need futher question about this properties here the host details:</strong><br/>
                                          Host Name          : {{$mail_data['owner_name']}}<br/>
                                          Host Contact Number: {{$mail_data['owner_phone']}}<br>
                                          Host Email         : {{$mail_data['owner_email']}}
                                    </p>
                                    @endif
                                    <!-- Salutation -->
                                    <p style="{{ $style['paragraph'] }}">
