...
@section('title')
    <title>{{get_string('users') . ' - ' . get_setting('site_name', 'site')}}</title>
@endsection
@section('content')
@section('page_title')
...
<div class="col l6 m8 s12 left left-align mbot10">
    {!!Form::open(['method' => 'post', 'url' => route('admin_user_search')]) !!}
    <div class="form-group col s8 autocomplete-fix">
        {{Form::text('term', null, ['class' => 'form-control', 'id' => 'term', 'placeholder' => get_string('search_users')])}}
    </div>
            ...
            <table class="table bordered striped">
                ...
                @foreach($users as $user)
                    <tr>
                        ...
                        <td>{{$user->username}}</td>
                        <td>{{$user->email}}</td>
                        <td>@if($user->user){{$user->user->first_name}} @else - @endif</td>
                        <td>@if($user->user){{$user->user->last_name}} @else - @endif</td>
...
<script>
    $(document).ready(function(){
        ...
        $('.mass-delete').click(function(event){
            ...
            $('#term').autocomplete({
                source: '{{ url('/admin/user/autocomplete') }}',
                minLength: 0,
                delay: 0,
