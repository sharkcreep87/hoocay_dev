                ...
                <div class="carousel-inner" role="listbox">
                    ...
                    @foreach($property->images as $image)
                        <div class="carousel-item  @if(!$c) active <?php $c++; ?> @endif ">
                            ...
                            <img class="" src="{{ URL::asset('images/data').'/'.$image->image }}" />
                        </div>
                        </div>
                    @endforeach
                </div>
        ...
        <div class="col-md-4 col-sm-12 sidebar">
            ...
            <div class="booking-form input-style filter-box mbot20">
                ...
                <p class="field-info text-centered mbot5">{{ $static_data['strings']['fill_fields_to_book'] }}</p>
                        ...
                        <input type="hidden" value="@if($static_data['user']){{ $static_data['user']->info->first_name }}@endif" name="first_name" required class="form-control filter-hidden hidden" placeholder="{{$static_data['strings']['your_name']}}">
                
           
                
                        <input type="hidden" value="@if($static_data['user']){{ $static_data['user']->email }}@endif" name="email" class="form-control filter-hidden hidden" required placeholder="{{$static_data['strings']['your_email']}}">
             
                    
                        <input type="hidden" value="@if($static_data['user']){{ $static_data['user']->info->phone }}@endif" required name="phone" class="form-control filter-hidden hidden" placeholder="{{$static_data['strings']['your_phone']}}">
                    ...
                    <div class="col-sm-12">
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                @if(isset($property->price_per_night))<tr><td>{{ $static_data['strings']['price_per_night'] }} </td><td class="price-per-night"><strong>{{ $property->price_per_night }}</strong></td></tr>@endif
                                @if(isset($property->fees['cleaning_fee']))<tr><td>{{ $static_data['strings']['cleaning_fee'] }} </td><td class="cleaning-fee"><strong>{{ 'RM' .$property->fees['cleaning_fee']}}</strong></td></tr>@endif
                                @if(isset($property->fees['city_fee']))<tr><td>{{ $static_data['strings']['city_fee'] }} </td><td class="city-fee"><strong>{{ 'RM' .$property->fees['city_fee']}}</strong></td></tr>@endif
                                 <tr> <td>Service charge</td><td><strong>{{$commission}} %</strong></td></tr>
                                <tr><td>Total </td>
            ...
            @else
                <p class="field-info text-centered mbot5">{{ $static_data['strings']['login_to_book'] }}
                 <a href="{{ route('login') }}" class="btn btn btn-info">{{ $static_data['strings']['sign_in'] }}</a>
                </p>
                 @endif
             @else 
