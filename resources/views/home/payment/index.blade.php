                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $currency.' '.number_format($fees, 2) }}</td>
                            <td>{{$currency.' '.number_format($charge, 2)}}</td>
                            <td>{{ $currency.' '.number_format($grand_total, 2) }}</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div class="col-sm-12">
            {!! Form::open(['method' => 'post', 'url' => route('booking_pay'), 'id' => 'payment-form']) !!}
                <div class="form-group  {{$errors->has('gateway') ? 'has-error' : ''}}">
                    {{Form::select('gateway', ['2' => 'Online Banking','0' => 'Paypal'], null, ['class' => 'form-control', 'id' => 'type', 'required', 'placeholder' => $static_data['strings']['choose_payment_method']] )}}
