...
class PaymentController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($days){
            ...
            $current_total = round(($total + $fees) * 100) / 100;

              $commission = Setting::where('key','host_commission')->pluck('value')->first();
              $charge = $current_total * $commission / 100;
                $grand_total = $current_total + $charge ;
            ...
            // Return View back
            return view('home.payment.index', compact('gateways', 'email', 'first_name', 'phone', 'owner_id', 'property_id', 'user_id', 'property', 'currency', 'static_data', 'guest_number', 'default_language', 'start_date', 'end_date', 'total', 'guest_number', 'fees', 'grand_total','charge'));
        }else{
...
//shafeq
        ...
        $this->price          = $grand_total;
        $this->phone          = $request->phone;

       // return response()->json($request->first_name);

            $billplz_req = \Billplz::issue(function ($bill)
    {
       
        $bill->to($this->name,$this->email,$this->phone)
             ->amount($this->price) // will multiply with 100 automatically, so a RM500 bill, you just pass 500 instead of 50000
             ->collectionid('ddfgj4aj')//collectionid('tkv5emcy')//->collectionid('ddfgj4aj')
             
             ->callbackUrl('https://hoocay.com/booking/payment/success_billplz')
             ->redirectUrl('https://hoocay.com/booking/payment/success_billplz')
             ->description('Booking for homestay');
    });

            Session::put('params', $params);
            Session::save();

           /* $gateway = $this->billplz_gw;
            $response = $gateway->purchase($params)->send();
            if ($response->isSuccessful()) {
                return true;
            }elseif($response->isRedirect()) {
             ...
             $billplz_res = response()->json($billplz_req);
             $data['billplz'] = $billplz_res->getData()->url;

      return redirect()->away($data['billplz']);
        }
    ...
    public function paymentSuccess()
    {
        ...
        if($params['gateway'] == 0){
            ...
            if(isset($paypalResponse['ACK']) && $paypalResponse['ACK'] === 'Success') {
               $user = Owner::where('user_id', $params['owner_id'])->first();
               $commission = Setting::where('key','host_commission')->pluck('value')->first();
               $total = $params['amount'] - $params['charge'];
               $serviceCharge = $params['charge'];
                if($user){
                    $user->pending_balance += $total;
                    $user->save();

                    $random1= rand(000000,999999);

                       $trans = new Transaction;



                        $trans->user_id = $params['owner_id'];
                        $trans->ref_no = "P-".$random1;
                        $trans->description = 'Received booking payment';
                        $trans->amount = $total;
                        $trans->save();
                        ...
                        $trans1->user_id = 1;
                        $trans1->ref_no = "P-".$random1;
                        $trans1->description = 'Received booking payment';
                        $trans1->amount = $total;
                        $trans1->save(); 

                        $trans2 = new Transaction;

                    $random2= rand(000000,999999);
                        $trans2->user_id = $params['owner_id'];
                        $trans2->ref_no = "P-".$random2;
                        $trans2->description = 'Service charges deduction';
                        $trans2->amount = $serviceCharge;
                        $trans2->state = 1;
                        $trans2->save();
                    ...
                    $trans3 = new Transaction;
                        ...
                        $trans3->user_id = 1;
                        $trans3->ref_no = "P-".$random2;
                        $trans3->description = 'Service charges received';
                        $trans3->amount = $serviceCharge;
                        $trans3->save();
                }
                $params['total']  = $params['amount'];
                $params['user_data']['first_name'] = $params['first_name'];
...
//shafeq
    public function paymentSuccessBillplz(Request $request)
    {
        $default_language = default_language();
        $static_data = static_home();
        ...
        $params = Session::get('params');
            ...
            $paid_status = $request->billplz['paid'];
     // $bill = \Billplz::get($request->billplz['id']);

      $response = \Billplz::get($request->billplz['id']);
        ...
        if($paid_status == 'true' ){
               ...
               $user = Owner::where('user_id', $params['owner_id'])->first();
                ...
                $params['booking_ref'] ='B-'.$random ;
                $booking = Booking::create($params);
                $params['booking_id'] = $booking->id;
                $params['transaction'] = $request->billplz['id'];
                $params['payment_method'] = 'Billplz';
                $params['host_commission'] = get_setting('host_commission', 'payment');
                Payment::create($params);
                ...
                $mail_data['str_total'] = $static_data['strings']['total'];
                $mail_data['mail_after_text_book'] = $static_data['strings']['mail_after_text_book'];
                $mail_data['ref'] = $params['booking_ref'] ;
                $mail_data['user_data'] = $params['user_data'];
                $owner = $booking->owner;
                $mail_data['owner_phone'] = $owner->info->phone;
                $mail_data['owner_email'] = $owner->email;
                $mail_data['owner_name']   = $owner->info->first_name;
                $mail_data['isOwner'] = 0;
